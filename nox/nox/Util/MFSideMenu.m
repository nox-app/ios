...
- (void)setHidesLeftSideMenu:(BOOL)a_bool
{
    m_hidesLeftSideMenu = a_bool;
}

- (void)setHidesRightSideMenu:(BOOL)a_bool
{
    m_hidesRightSideMenu = a_bool;
}

- (void) handleRightPan:(UIPanGestureRecognizer *)recognizer {
    if(!self.leftSideMenuViewController) return;
    if(m_hidesLeftSideMenu) return;
...
- (void) handleLeftPan:(UIPanGestureRecognizer *)recognizer {
    ...
    if(m_hidesRightSideMenu) return;
